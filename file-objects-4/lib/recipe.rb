...
class Custom < Recipe
  def to_json
    ...
    if featured == true then
      Custom.new(title: @title, description:@description, ingredients:@ingredients, cook_time:@cook_time, featured: @feature)
      File.write('data/recipe.json', "{\"title\":\"[Destaque] #{title}\",\"description\":\"#{description}\",\"ingredients\":\"#{ingredients}\",\"cook_time\":\"#{cook_time} minutos\"}").to_json
      File.open('data/recipe.json')
    else
      save_to_file
    end
  end
...
Recipe = Custom
custom = Custom.new(title:@title, description:@description, ingredients:@ingredients, cook_time:@cook_time)
